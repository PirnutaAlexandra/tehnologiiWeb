...
window.getById = async function (e) {
  e.preventDefault();                        
  const id = document.getElementById('inputId').value.trim();
  if (!id) { alert('Introdu un ID!'); return; }

  console.log('Caut ID:', id);                  

  try {
    const person = await get(apiUrl + 'getId/' + parseInt(id, 10));
    document.getElementById('result').innerHTML =
      `<p><b>Nume:</b> ${person.name}, <b>Vârstă:</b> ${person.age} <i>(id ${person.id})</i></p>`;
  } catch (err) {
    console.error(err);
    document.getElementById('result').innerHTML =
      `<p style="color:red">Persoana cu ID ${id} nu a fost găsită.</p>`;
  }
};
